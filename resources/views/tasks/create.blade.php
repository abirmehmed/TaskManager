...
<div class="max-w-2xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
    <div class="bg-white/10 backdrop-blur-md rounded-2xl shadow-xl p-8 border border-white/20">
        <h1 class="text-3xl font-bold text-white mb-2">Create a New Task</h1>
        <p class="text-white/60 mb-6">Get things done, one step at a time.</p>
        ...
        <form method="POST" action="{{ route('tasks.store') }}">
            ...
            <div class="mb-5">
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-2 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-white/50">{{ old('description') }}</textarea>
            ...
            <div class="flex justify-end space-x-3">
                ...
                <button type="submit" class="bg-white/30 hover:bg-white/40 text-white font-semibold px-5 py-2 rounded-lg transition shadow-md">Save Task</button>
            </div>
        </form>
    </div>
</div>
